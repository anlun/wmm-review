...
If the accesses to mutable shared variables are not protected by locks 
or other syncronization primitives, then program can contain \emph{data-races}.
...
(we enclose accesses to shared locations into square brackets, in order to distinguish them from accesses to registers).
...
and memory models that allows such behaviors also called \emph{weak models}.
...
Hardware and programming languages putdifferent requirements on a memory model, 

...
Some memory models furhter distinguish 
...
They are denoted as $\rlx$, $\acqrel$, and $\sco$ correspondingly.
...
Non-atomic accesses, as we have already discussed, 
give little guarantee. 
...
threads synchronize their view on shared memory. 
...
fragment of the \ref{ex:Dekker} program from \cref{sec:intro}.
...
to compile it to \xTSO hardware. 
...
Note that the list is far from being a complete regarding to  

...
\todo{Explain why we count transformation as sound in the table. 
Tip: we borrow everything from SC because the authors prove 
their semantics is equivalent to SC.
Note in the formal model they assume TSO like 
underlying hardware model.}
...
\todo{Explain why we count reorderings as unsound in the table. 
Tip: because of catch-fire semantics.}
...
\todo{We will not assume soundness of transformations from the fact 
that some of them are ``baked into the model''.}
...
\todo{Mention that semantics of plain accesses does not enforce coherence.}

...
However, the transformations can only be performed
...
Given the evalution of performance regress above,
one can argue that the cost is worth the prize
of having simple SC-like model.
However, we point out that the solution above 
require non-trivial modifications to the 
existing hardware and compilers, or, 
in case of SC-Haskell, a strict programming 
discipline enforced by the type system of the language.
Moreover, the in expierements the proposed 
solutions were mainly compared to x86 hardware, 
...
By taking a relatively simple and yet practicle memory model
as a first target they hoped to made this task feasible. 
The authors proved soundness of several program transformations
(including store/load reordering, elimination of redundunt
load/load, store/store, store/load pairs and several others, 
see the table~\cite{table:summary} for details
\footnote{\eupp{If we'll decide to mention thread inlining w.r.t. TSO
then cite~\cite{Lahav-Vafeiadis:FM16} 
(since the paper itself doesn't mention this transformation)}}).


\cite{Boudol-el:POPL09}

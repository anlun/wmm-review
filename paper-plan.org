...
Otherwise she must be very careful because accesses to mutable shared variables, 
that are not protected by locks or other syncronization primitives, can lead to data-races, 
a situation when two threads concurrently access the same variable and at least one of them is performing a write operation.
...
As we have seen, the modern CPU's do not provide us sequentially consistent memory model. 
Memory models of programming languages also cannot provides us guarantees of sequential consistency
without a sacrifice of of the compiled code's performance .  
